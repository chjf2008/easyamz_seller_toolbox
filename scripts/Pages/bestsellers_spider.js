...

{
    ...
    formatter: function (value, row, index) {
        ...
        let item = '';
        if (row.title) {
            item = "<span style='max-width:300px;word-break:normal; width:auto; display:block; white-space:pre-wrap;word-wrap : break-word ;overflow: hidden'>" + row.title + "</span>";
...
},
{
    ...
    formatter: function (value, row, index) {
        ...
        item = "<span style='max-width:500px;word-break:normal; width:auto; display:block; white-space:pre-wrap;word-wrap : break-word ;overflow: hidden'>" + row.title + "</span>";
        }
...
},
{
    ...
    formatter: function (value, row, index) {
        if (value) {
            return value;
        } else if (row.sold_by_amz == true || (row.title && row.title.indexOf('Amazon') > -1)) {
            return 'Amazon';
        }
        return '';
    },
...
$(document).ready(function () {
    ...
    API.APP.init_page(CONST.app_const.top100.key).then(function () {
        ...
        //$('#id_cloud_progress_bar').find("i").click();
...
function start_crawl() {
    ...
    let include_children = $("#id_crawl_children").prop("checked");
    if (!start_url) {
        return;
    }
    const uri = new URL(start_url);

    const host = uri.host;
    if (!/^www\.amazon.(com|ca|com\.mx|com\.br|co\.uk|de|fr|it|es|nl|co\.jp|com\.au|sg|sa|ae)$/.test(host)) {
        Helper.functions.show_msg(msg_container, '不支持的URL', 'error');
        return;
    }
    let marketplaceId = Helper.functions.get_marketplaceId_by_domain(host);
    ...
    Helper.functions.show_msg(msg_container, '正在验证链接有效性...');
    Helper.request.get(start_url).then(function (html) {
        const uri = new URL(start_url);
        if (html.indexOf('captchacharacters') > -1) {
        ...
        $('#id_btn_stop').text('暂停').show().off('click').on('click', function (e) {
            pause_crawl(e);
        });
        let result = portal_process(window.global_jobs, start_url, include_children, '', html);
        if (result &&
            result.hasOwnProperty('tasks') &&
            result.hasOwnProperty('top_category')) {
            let top_category = result.top_category;
            let items = result.items;
            window.global_jobs = result.tasks;

            if (top_category && window.global_jobs && window.global_jobs.length > 0) {
                Helper.functions.show_msg(msg_container, '验证成功,正在下载数据...');
                obj['top_category'] = top_category;
                Helper.functions.save_config(obj).then(function (config) {
                    if (items && items.length > 0) {

                        $('#id_table').bootstrapTable('prepend', items);
                        //存储数据
                        window.db.items.bulkPut(items).then(function () {
                            get_data_count().then(function (count) {
                                if (count > 0) {
                                    let text = '请勿刷新网页,已成功下载' + count + '条数据，<br/>正在下载:' + start_url;
...
function portal_process(tasks, start_url, include_children, top_category, html) {
    ...
    const uri = new URL(start_url);
    let tree = [];
    let categroy_selected = '';
    let $categroy_selected = $doc.find("span[class*='zg-selected']");
    if ($categroy_selected.length == 1) {
        categroy_selected = $.trim($categroy_selected.html());
        if (categroy_selected) {
            ...
            let $group = $categroy_selected.closest("div[role='group']");
            if ($group.length == 1) {
            ...
            //上级分类们
            const categories = tree.map(function (item) {
                return item.category;
            });

            top_category = top_category ? top_category : (categories.length > 0 ? categories[0] : '');
            ...
            if ($next_page.length == 1) {
                ...
                if (include_children) {
                    ...
                    let child_items_links = $categroy_selected.parent("div[role='treeitem']").next("div[role='group']").find("div[role='treeitem'] a");
                    $.each(child_items_links, function (i, ele) {
                        tasks.push({
                            'tree': tree,
                            'url': uri.origin + $(ele).attr('href'),
                            'include_children': include_children,
                            'page': 1
                        });
                    });
            ...
            //获取当前页页码
            let page = $doc.find('li.a-selected a').html();
            if (page == "1" || page == '2') {
                page = parseInt(page);
            }
            //处理第一页数据
            ...
            if (str_ids) {
                ...
                for (let idx in obj_ids) {
                    let asin = obj_ids[idx].id;

                    let metadataMap = obj_ids[idx].metadataMap;
                    if (metadataMap) {
                        let rank = metadataMap['render.zg.rank'];

                        items.push({

                            'id': Helper.string.uuid(), 'asin': asin, 'rank': rank, "tree": tree,
                            "domain": uri.host,'completed':0
                ...
                for row in obj_list:
                asin = row.get("id")
                rank = row.get("metadataMap").get("ender.zg.rank")
                data.append({"asin": asin, "rank": rank,'domain':uri.netloc, "tree": tree}) */
            }
    ...
    Helper.functions.get_config().then(function (config) {
        if (config.paused == true) {
            ...
            if (jobs.length == 0) {
                ...
                let job = jobs.shift();
                let page = job.page;
                let include_children = job.include_children;
                let url = job.url;
                ...
                Helper.request.get(url).then(function (html) {
                    ...
                    if (html.indexOf('twotabsearchtextbox') > -1 && html.indexOf('captchacharacters') == -1) {
                        return {
                            'result': 1,
                            'html': html,
                            'jobs': jobs,
                    ...
                    if (save_jobs === true) {
                        Helper.functions.get_config().then(function (config) {
                            ...
                            $('#id_btn_stop').text('继续').show().off('click').on('click', function (e) {
                                continue_crawl();
                            });
                    ...
                    if (res.result == 1) {
                        ...
                        const result = portal_process(res.jobs, res.url, res.include_children, base_path, res.html);

                        let jobs = result['tasks'];
                        ...
                        let categories = tree.map(function (item) {
                            return item.category;
                        });


                        if (categories.indexOf(top_category) > -1) {
                            if (items && items.length > 0) {
                                ...
                                window.db.items.bulkPut(items).then(function () {
                                    get_data_count().then(function (count) {
                                        if (count > 0) {
                                            let text = '请勿刷新网页,列队数量:' + jobs.length + ',已成功下载' + count + '条数据，<br/>正在下载:' + url;
                                            Helper.functions.show_msg(msg_container, text, 'success', false);
                                        }
                                        loop(jobs, top_category);
                                    });
                                });
                            } else {
                                loop(jobs, top_category);
                            ...
                            Helper.functions.show_msg(msg_container, '丢弃：' + res.url);
                            loop(jobs, top_category);
                        }
                    }
                }).catch(function (error) {
                    ...
                    if (error.status == 404) {
                        ...
                        save_jobs = true;
                        Helper.functions.show_msg(msg_container, '请稍后再试!', 'error');
                    } else {
                        save_jobs = true;
                        Helper.functions.show_msg(msg_container, '未知错误，请检查网络！', 'error');
                    }
                    if (save_jobs) {
                        Helper.functions.get_config().then(function (config) {
                            jobs.push(_job);
                            ...
                            task['config'] = config;
                            task['jobs'] = jobs;
                            window.localStorage.setItem('temp_job', JSON.stringify(task));
